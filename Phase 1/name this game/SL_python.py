...
def save_results(fieldnames,total_time,call_back_time,test_acc,test_loss):
    ...
    with open('/home-mscluster/fmahlangu/2089676/atari_name_this_game_data/results.csv', 'a', encoding='UTF8') as f:
...
def save_model(fieldnames,first_layer,second_layer,third_layer):
    ...
    with open('/home-mscluster/fmahlangu/2089676/atari_name_this_game_data/models.csv', 'a', encoding='UTF8') as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writerows(rows)
...
def get_data(trial):
    ...
    game_folder= str("/home-mscluster/fmahlangu/2089676/atari_name_this_game_data/")
...
def main():
    ...
    with open('/home-mscluster/fmahlangu/2089676/atari_name_this_game_data/results.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames_results)
        writer.writeheader()

    #prepare csv for results 
    fieldnames_m = ['first_layer','second_layer','third_layer'] 
    with open('/home-mscluster/fmahlangu/2089676/atari_name_this_game_data/models.csv', 'w', encoding='UTF8', newline='') as f:
    ...
    metaData_df= pd.read_csv('/home-mscluster/fmahlangu/2089676/atari_name_this_game_data/meta_data.csv')

    dataframe= metaData_df[metaData_df.GameName=="name_this_game"]
    ...
    params = []
    # params.append([[52, 5, 2, 'same', 'relu', 2, 0.02, 512, 0.03, 0.0003],[16, 5, 1, 'same', 'relu', 2, 0.15],[32, 3, 1, 'same', 'relu', 3, 0.25]])
    params.append([[18, 2, 1,'same', 'relu', 2, 0.01, 512, 0.15, 0.000003],[50, 1, 1, 'same', 'relu', 2, 0.15],[128, 3, 1,'same', 'relu', 3, 0.2]])
    params.append([[16, 2, 1,'same', 'relu', 2, 0.05, 128, 0.05, 0.02],[32, 4, 2,'same', 'relu', 3, 0.15],[52, 5, 1,'same', 'relu', 2, 0.15]])
    params.append([[32, 3, 1,'same', 'relu', 2, 0.25, 128, 0.25, 0.00005],[64, 2, 2,'same', 'relu',  3, 0.15],[256, 5, 1,'same', 'relu', 2, 0.35]])
    params.append([[32, 4, 1, 'same', 'relu',2, 0.03, 64, 0.03, 0.004],[64, 3, 3,'same', 'relu', 2, 0.15],[512, 3, 1,'same', 'relu', 3, 0.05]])
